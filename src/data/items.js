...
export const ITEMS = {
  ...
  b: { sprite: 'soulsphere', spriteH: 0.55, sound: 'health',
    apply: (g) => { g.player.health = Math.min(200, g.player.health + 100); g.player.powerFlash = 1; g.message('Supercharge! (+100)'); return true; } },
  u: { sprite: 'radsuit', spriteH: 0.6, sound: 'pickup',
    apply: (g) => { g.player.radsuit = 45; g.player.powerFlash = 0.6; g.message('RADIATION SHIELDING SUIT'); return true; } },
  I: { sprite: 'quad', spriteH: 0.55, sound: 'health',
    apply: (g) => { g.player.quad = 30; g.player.powerFlash = 1; g.player.faceMood = 'angry'; g.player.faceTimer = 1; g.message('QUAD DAMAGE!'); return true; } },
  V: { sprite: 'invuln', spriteH: 0.55, sound: 'health',
    apply: (g) => { g.player.invuln = 30; g.player.powerFlash = 1; g.message('INVULNERABILITY!'); return true; } },
  k: { sprite: 'berserk', spriteH: 0.5, sound: 'health',
    apply: (g) => { g.player.berserk = true; g.player.health = Math.max(g.player.health, 100); g.player.damageFlash = Math.min(1, g.player.damageFlash + 0.9); if (g.player.weapon !== 0) g._switchWeapon(0); g.message('BERSERK!'); return true; } },
  v: { sprite: 'visor', spriteH: 0.45, sound: 'pickup',
    apply: (g) => { g.player.visor = 60; g.message('Light Amplification Visor'); return true; } },
  ...
  0: { sprite: 'pickup_rail', spriteH: 0.45, sound: 'weapon',
    apply: (g) => giveWeapon(g, 8, 'cells', 40, 'You got the RAILGUN!') },
};
